...
const eventIdValidator = [
  param("eventId")
    .notEmpty()
    .withMessage("ID événement requis")
    .isUUID()
...
const updateRateLimitValidator = [
  ...
  body("max")
    .isInt({ min: 1, max: 50 })
    .withMessage("Max doit être entre 1 et 50"),
  body("window")
    .isInt({ min: 1, max: 120 })
    .withMessage("Window doit être entre 1 et 120 minutes"),
];
...
const spotifySearchValidator = [
  query("q")
    .trim()
    .notEmpty()
    .withMessage("Query de recherche requise")
    .isLength({ min: 2, max: 100 })
    .withMessage("Query doit contenir entre 2 et 100 caractères"),
  query("eventId")
    .notEmpty()
    .withMessage("EventId requis")
    .isUUID()
