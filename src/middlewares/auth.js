...
/**
 * Accepte le DJ propriétaire OU un modérateur authentifié pour cet événement.
 * Utilisé sur les routes auxquelles les deux ont accès (ex: add-song-dj).
 */
const requireModOrOwnership = async (req, res, next) => {
  try {
    const { eventId } = req.params;

    // Modérateur avec session pour cet event
    ...
    const [rows] = await db.query(
      "SELECT dj_id FROM events WHERE id = ?",
      [eventId],
    );
    if (rows.length === 0) {
      return res.status(404).json({ error: "Événement non trouvé" });
    ...
    console.error("Erreur requireModOrOwnership:", error);
    res.status(500).json({ error: "Erreur serveur" });
  }
};
